 ...
 * FolderSortFunction
 * @description
 * FolderSortFunction is a type that represents a function used to sort an array of FolderEntry objects.
 * It takes two FolderEntry objects as arguments and returns a number indicating their relative order.
 ...
 *
 * (JP)
 * FolderSortFunctionは、FolderEntryオブジェクトの配列をソートするために使用される関数を表す型です。
 * 2つのFolderEntryオブジェクトを引数として受け取り、それらの相対的な順序を示す数値を返します。
